...
if($num> 0){
    ...
    while($row = mysqli_fetch_assoc($result)){
        ...
        <td><a href = 'update_criminal_record.php?cid=$row[Criminal_ID] &
        nm=$row[Name] & ag=$row[Age] & gn=$row[Gender] &
        db=$row[Date_of_Birth] & ht=$row[Height] &
        bg=$row[Blood_Group] & cc=$row[Crime_Committed] &
        pa=$row[Prior_Arrest] & ad=$row[Address] &
        ph=$row[Phone] & jn=$row[Jail_Number] &
        fn=$row[FIR_Number]'>Update</td>
        </td>
        </tr>
        ";
    }
